...
// Last Modified: Tue Dec 19 19:29:03 PST 2017
// Filename:      CheckSum.cpp
// Syntax:        C++
// vim:           ts=3:nowrap
//
...
string CheckSum::getMD5Sum(vector<vector<unsigned char>>& data) {
	...
	for (int i=0; i<(int)data.size(); i++) {
		int count    = data[i].size() / 64;
		int leftover = data[i].size() % 64;
		for (int j=0; j<count; j++) {
			MD5Update(&context, data[i].data() + j * 64, 64);
		}
		if (leftover > 0) {
			MD5Update(&context, data[i].data() + count*64, leftover);
		}
	}
...
string CheckSum::getMD5Sum(const string& data) {
	MD5_CTX context;
	MD5Init(&context);

	int i;
	int count    = data.size() / 64;
	int leftover = data.size() % 64;

	for (i=0; i<count; i++) {
		MD5Update(&context, (unsigned char*)data.data() + i * 64, 64);
	}
	if (leftover > 0) {
		MD5Update(&context, (unsigned char*)data.data() + count*64, leftover);
	}
...
void CheckSum::getMD5Sum(ostream& out, stringstream& data) {
	...
	int i;
	int count    = buffer.size() / 64;
	int leftover = buffer.size() % 64;

	for (i=0; i<count; i++) {
		MD5Update(&context, (unsigned char*)buffer.data() + i * 64, 64);
	}
	if (leftover > 0) {
		MD5Update(&context, (unsigned char*)buffer.data() + count * 64, leftover);
	}
